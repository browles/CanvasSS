...
var layer1 = new CanvasSS(id('container'), id('layer1'), { ratio: 1 });
var layer2 = new CanvasSS(id('container'), id('layer2'), { ratio: 1 });
...
layer2.select('.hover', function (ctx, cursor) {
  var w = cursor.width;
  var h = cursor.height;
  var s = Math.floor(cursor.time / 100) % colors.length;
...
var hue = 1;
layer2.select('.fire', function (ctx, cursor) {
  hue = (hue + 0.5) % 360;
  particles.forEach(function (p, i) {
    ctx.beginPath();
    var x = p.location.x * cursor.width;
    var y = p.location.y * cursor.height;
...
container.onmousemove = function (e) {
  var _layer2$getRenderCont = layer2.getRenderContext(e.target);

  var x = _layer2$getRenderCont.x;
  var y = _layer2$getRenderCont.y;
...
qs('.cell').forEach(function (div) {
  ...
  div.onclick = function (e) {
    div.classList.remove('clicked');
    var rctx = layer2.getRenderContext(div);
